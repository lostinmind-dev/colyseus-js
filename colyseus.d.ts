...
import { Packr } from 'npm:@colyseus/msgpackr';
import * as httpie from 'npm:httpie';
...
declare namespace Colyseus {
	...
	class Connection implements ITransport {
	...
	const $track: unique symbol;
	const $encoder: unique symbol;
	const $decoder: unique symbol;
	const $filter: unique symbol;
	const $getByIndex: unique symbol;
	const $deleteByIndex: unique symbol;
	const $changes: unique symbol;
	const $onEncodeEnd: unique symbol;
	const $onDecodeEnd: unique symbol;
	const $descriptors: unique symbol;
	...
	class Encoder<T extends Schema = any> {
	...
	class Decoder<T extends Schema = any> {
		context: TypeContext;
		state: T;
	...
	class MapSchema<V = any, K extends string = string> implements Map<K, V>, Collection<K, V, [
		K,
		V
	]> {
	...
	class SetSchema<V = any> implements Collection<number, V> {
	...
	class ChangeTree<T extends Ref = any> {
		ref: T;
		refId: number;
		root?: Root;
		parent?: Ref;
	...
	class CollectionSchema<V = any> implements Collection<K, V> {
		protected $items: Map<number, V>;
		protected $indexes: Map<number, number>;
		protected deletedItems: {
			[field: string]: V;
	...
	class ArraySchema<V = any> implements Array<V>, Collection<number, V> {
		[n: number]: V;
		protected items: V[];
	...
	export function getStateCallbacks<T extends Schema>(room: Room<T>): SchemaCallbackProxy<T>;
	...
	class HTTP {
		...
		get<T = any>(path: string, options?: Partial<httpie.HttpieOptions>): Promise<httpie.HttpieResponse<T>>;
		post<T = any>(path: string, options?: Partial<httpie.HttpieOptions>): Promise<httpie.HttpieResponse<T>>;
		del<T = any>(path: string, options?: Partial<httpie.HttpieOptions>): Promise<httpie.HttpieResponse<T>>;
		put<T = any>(path: string, options?: Partial<httpie.HttpieOptions>): Promise<httpie.HttpieResponse<T>>;
		protected request(method: "get" | "post" | "put" | "del", path: string, options?: Partial<httpie.HttpieOptions>): Promise<httpie.HttpieResponse>;
		protected getOptions(options: Partial<httpie.HttpieOptions>): Partial<httpie.HttpieOptions>;
	}
